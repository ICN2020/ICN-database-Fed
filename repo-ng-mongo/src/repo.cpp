...
#include "storage/mongo-storage.hpp"
...
parseConfig(const std::string& configPath)
{
  ...
  if (repoConf.get<std::string>("storage.method") != "mongo") {
    BOOST_THROW_EXCEPTION(Repo::Error("Only 'mongo' storage method is supported"));
  }
...
Repo::Repo(boost::asio::io_service& ioService, const RepoConfig& config)
  ...
  , m_face(ioService)
  , m_store(std::make_shared<MongoStorage>(config.dbPath,*m_config.dataPrefixes.begin()))
  , m_storageHandle(config.nMaxPackets, *m_store)
  , m_validator(m_face)
  , m_readHandle(m_face, m_storageHandle, m_keyChain, m_scheduler,*m_config.dataPrefixes.begin())
