...
const size_t MAX_NDN_PACKET_SIZE = ndn::MAX_NDN_PACKET_SIZE;
                                           ...
                                           const shared_ptr<detail::TcpBulkInsertClient>& client)
{
  ...
  while (m_inputBufferSize - offset > 0) {
    ...
    if (element.type() == ndn::tlv::Data) {
      try {
        ...
        if (isInserted)
          std::cerr << "Successfully injected " << data.getName().toUri() << " size: "<< data.getContent().size()<< std::endl;
        else
        ...
        std::cerr << "Error decoding received Data packet " << error.what() <<  std::endl;
      }
    }
  }
  ...
  if (!isOk && m_inputBufferSize == MAX_NDN_PACKET_SIZE && offset == 0)
    {
      std::cerr << "NOT OK in handleReceive" << std::endl;
      boost::system::error_code error;
      m_socket->shutdown(boost::asio::ip::tcp::socket::shutdown_both, error);
      m_socket->close(error);
      return;
